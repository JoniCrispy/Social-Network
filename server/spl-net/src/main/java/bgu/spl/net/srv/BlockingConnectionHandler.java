package bgu.spl.net.srv;
import bgu.spl.net.api.bidi.BidiMessagingProtocol;
import bgu.spl.net.api.MessageEncoderDecoder;
import bgu.spl.net.api.MessagingProtocol;
import bgu.spl.net.impl.BGSServer.BGSEncoderDecoder;
import bgu.spl.net.impl.BGSServer.BGSprotocol;
import bgu.spl.net.impl.BGSServer.ConnectionImpl;
import bgu.spl.net.impl.BGSServer.User;
import bgu.spl.net.impl.rci.Command;

import java.io.BufferedInputStream;
import java.io.BufferedOutputStream;
import java.io.IOException;
import java.net.Socket;

public class BlockingConnectionHandler<T> implements Runnable, bgu.spl.net.srv.ConnectionHandler<T> {

    private final BGSprotocol protocol;
    private final BGSEncoderDecoder encdec;
    private final Socket sock;
    private BufferedInputStream in;
    private BufferedOutputStream out;
    private volatile boolean connected = true;

    public BlockingConnectionHandler(Socket sock, BGSEncoderDecoder reader, BGSprotocol protocol) {
        this.sock = sock;
        this.encdec = reader;
        this.protocol = protocol;
    }

    @Override
    public void run() {
        try (Socket sock = this.sock) { //just for automatic closing
            int read;

            in = new BufferedInputStream(sock.getInputStream());
            out = new BufferedOutputStream(sock.getOutputStream());
            while (!protocol.shouldTerminate() && connected && (read = in.read()) >= 0) {
                char[] nextMessage = encdec.decodeNextByte((byte) read);
                if (nextMessage != null) {
                    Command<User> newMessage = ConnectionImpl.getInstance().decodeToCommand(nextMessage);
                    protocol.process(newMessage);
//                    if (response != null) {                **we dont get response from process.**
//                        out.write(encdec.encode(response));
//                        out.flush();
//                    }
                }
            }

        } catch (IOException ex) {
            ex.printStackTrace();
        }

    }
    
    @Override
    public void close() throws IOException {
        connected = false;
        sock.close();
    }
    
    @Override
    public void send(T msg) {
        if (msg != null) {                    //Just a sanity check that we didnt send null msg.
            try {
                out.write(encdec.encode(msg));
                out.flush();
            } catch (IOException ex) {
                ex.printStackTrace();
            }
        }

    }        
    
}
